...
func Middleware(config Config) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if config.SubDictKey != "" { // Start a new event (dict) if there is sub key.
				evt = zerolog.Dict()
			} else {
				evt = mainEvt
			}
			...
			if config.SubDictKey != "" { // Nest the new event (dict) under the sub key.
				mainEvt.Dict(config.SubDictKey, evt)
			}
			evt.Send()
